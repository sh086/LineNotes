# Blog

## 知识点

- [数值比较==与equal](./2021/数值比较==与equal.html)
- [JSON字符串与属性名不匹配问题](./2022/JSON字符串与属性名不匹配问题.md)
- [运行时动态调整Application参数值](./2021/运行时动态调整Application参数值.html)



## Spring

### SpringFram
- [SpringBoot整合MybatisPlus](./2021/SpringBoot整合MybatisPlus.html)



### Spring注解

- [@Builder构建者模式](./2021/@Builder构建者模式.html)
- [@Scheduled定时任务](./2021/SpringBoot启用定时任务.html)
- [动态化开启定时任务](./2021/动态化开启定时任务.html)
- [SpringBoot启用多线程](./2021/SpringBoot启用多线程.html)




## Component
- [百度文字识别](./2021/百度文字识别.html)



## Devops

- [在jar包中打包lib目录](./2022/在jar包中打包lib目录.md)
- [通过接口运维数据库](./2021/通过接口运维数据库.html)
- [Jmeter压力测试](./2021/Jmeter压力测试.html)
- [使用Alibaba canal同步数据库](./2021/使用AlibabaCanal同步数据库.html)
- [swap交换分区解决内存不足问题](./2021/swap交换分区解决内存不足问题.html)
- [screw数据库文档生成工具](./2021/screw数据库文档生成工具.html)
- [快速部署SkyWalking监控](./2021/快速部署SkyWalking监控.html)



